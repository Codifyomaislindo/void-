...
local function loadLibrary(url)
    ...
    error("Failed to load library: "..url)
end
...
local function updateESP()
    ...
    for _, fruit in pairs(Workspace:GetChildren()) do
        if fruit:IsA("Model") and fruit:FindFirstChild("Handle") and fruit.Name:find("Fruit") and not fruit.Name:find("Boat") then
            if not fruit:FindFirstChild("FruitESP") then
                local billboard, distanceLabel = createESP(fruit)
                RunService.RenderStepped:Connect(function()
                    if not espEnabled or not fruit.Parent or not fruit:FindFirstChild("Handle") then
                        billboard:Destroy()
                        return
                    end
                    local playerPos = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and LocalPlayer.Character.HumanoidRootPart.Position
                    if playerPos then
                        local distance = (playerPos - fruit.Handle.Position).Magnitude
    ...
    if mobInstance and mobInstance:FindFirstChild("HumanoidRootPart") and mobInstance:FindFirstChildOfClass("Humanoid") then
        pcall(function()
            ...
            while autoFarmEnabled and mobInstance and mobInstance:FindFirstChildOfClass("Humanoid") and mobInstance.Humanoid.Health > 0 do
                local combatFramework = LocalPlayer.PlayerScripts:FindFirstChild("CombatFramework")
                if combatFramework then
                    local combat = require(combatFramework)
